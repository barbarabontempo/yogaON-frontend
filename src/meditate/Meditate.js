...
state = {
  ...
  render() {
    ...
    const timeOptions = this.state.timeValues.map((duration) =>
      <button id="btn-med" key={duration} onMouseEnter={this.handleTimeHover} onMouseLeave={this.handleTimeHover} className={ !this.state.timeHovered && duration === this.state.desiredTime 
                                          ? "active" : "" } onClick={ () => {this.timeSelect({duration})} }>{duration/60} Minutes</button>
    );

    const audioOptions = this.state.audioNames.map((audioName) =>
      <button id="btn-med" key={audioName} onMouseEnter={this.handleAudioHover} onMouseLeave={this.handleAudioHover} className={ !this.state.audioHovered && this.state.audioUrl === "audio/" + audioName.toLowerCase() + ".mp3" 
    ...
    return (
      <>
            ...
            {/* <img id="background-video-med" src={require('../gifs/meditatevid-unscreen.gif')} alt="loading..." /> */}
